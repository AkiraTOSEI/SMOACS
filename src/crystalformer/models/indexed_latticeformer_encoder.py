...
from torch.nn import Dropout, Identity, LayerNorm, Linear, Module, ModuleList, Parameter
from torch.nn.init import constant_, xavier_normal_, xavier_uniform_

from .indexed_lattice_multi_head_attention import IndexedLatticeMultiheadAttention
from .indexed_lattice_multi_head_attention_cuda import (
    CrystalformerMultiheadAttentionCUDA,
)
...
def compute_lattice_distances(
    ...
    if exclude_self:
        ...
        self_edges = torch.cat(
            [
                torch.arange(s, **factory_kywd_long) * (1 + s) + c
                for s, c in zip(sizes, c2)
            ]
        )
    ...
    if output_rec:
        ...
        # [<r,b1>, <r,b2>, <r,b3>]
        cos_kr = (
            pos_p_q[:, None] @ recip_vec.transpose(1, 2)[b2e]
        )  # (E,1,D)   = (E, 1,D)x(E, D,D)
...
def compute_lattice_distances_(
    pos, batch, trans_vec, lattice_range, output_dis=True, output_rec=False
):
    # pos: (samples, sdim)
    # batch: (sammples, )
    ...
    for i, p in enumerate(pos):
        num = p.shape[0]
        rel_pos = p.reshape(1, num, sdim) - p.reshape(
            num, 1, sdim
        )  # (q-sam, k-sam, sdim)
    ...
    rel_dis_rep = (
        torch.cat([x for x in rel_dis_rep], dim=0) if rel_dis_rep is not None else None
    )
    rec_cos_kr = (
        torch.cat([x for x in rec_cos_kr], dim=0) if rec_cos_kr is not None else None
    )
...
class IndexedLatticeformerEncoder(Module):
    ...
    def __init__(
        ...
        self.layers = ModuleList(
            [
                IndexedLatticeformerEncoderLayer(
                    d_model,
                    nhead,
                    dim_feedforward,
                    dropout,
    ...
    def forward(
        ...
        #
        batch = batch - batch[0]
        dist2, cos_kr, k2, vcell, selfe, edges, pe_wave = compute_lattice_distances(
            pos,
            batch,
            trans,
            sizes,
            self.lattice_params.lattice_range,
            output_dis=self.lattice_params.domain
            in (
                "real",
                "multihead",
                "real-reci",
            ...
            output_rec=self.lattice_params.domain
            in (
                "reci",
                "multihead",
                "real-reci",
            ...
            dim_pe_wave=max(
                self.lattice_params.value_pe_wave_real,
                self.lattice_params.value_pe_wave_reci,
            ),
        )
        ...
        if dist2 is not None:
            ...
            # dist2 *= dist2 # 元々書かれていたコード
            dist2_clone = dist2.clone()
            dist2 = dist2_clone * dist2_clone
        if k2 is not None:
        ...
        for mod in self.layers:
            output = mod(
                output, batch, edges, dist2, cos_kr, k2, vcell, selfe, pe_wave, onehot
            )
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            x = self.norm1(
                x
                + self._sa_block(
                    x, batch, edges, dist2, cos_kr, k2, vcell, selfe, pe_wave, onehot
                )
            )
...
class CrystalformerEncoderCUDA(Module):
    ...
    def forward(
        ...
        (
            ...
            vcell,
            edges,
        ) = compute_lattice_distances_for_cuda(
            pos,
            batch,
            trans,
            sizes,
            ...
            output_real=self.lattice_params.domain
            in (
                "real",
                "multihead",
                "real-reci",
            ...
            output_reci=self.lattice_params.domain
            in (
                "reci",
                "multihead",
                "real-reci",
